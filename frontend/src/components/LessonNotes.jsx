...
export default function LessonNotes({ lessonId }) {
  const [editing, setEditing] = useState(false), [savedHtml, setSavedHtml] = useState(""), [hasContent, setHasContent] = useState(false), [status, setStatus] = useState("");
  ...
  useEffect(() => {
    ...
    api.getNote(lessonId).then(n => { const h = authorizeImages(n?.content_html || ""); setSavedHtml(h); editor.commands.setContent(h || "<p></p>"); setHasContent(!editor.isEmpty); setStatus(""); }).catch(e => setStatus(e.message));
  }, [lessonId, editor]);
  ...
  const save = async () => { setStatus("Saving…"); try { const r = await api.saveNote(lessonId, stripTokens(editor.getHTML())); const h = authorizeImages(r.content_html || ""); setSavedHtml(h); editor.commands.setContent(h || "<p></p>"); setHasContent(!editor.isEmpty); setEditing(false); setStatus("Saved"); } catch(e) { setStatus(e.message); } };
  ...
  return <section className="lesson-notes card">
    ...
    <EditorContent editor={editor} className={!editing && !hasContent ? "notes-empty" : ""}/>
    {!editing && !hasContent && <p className="empty-message">No notes yet. Pause the video and add key points or screenshots here.</p>}
